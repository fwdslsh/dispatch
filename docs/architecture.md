...
class SessionAdapter {
  ...
  async create(runId, options) { throw new Error('Not implemented'); }
  async destroy(runId) { throw new Error('Not implemented'); }
  async handleInput(runId, input) { throw new Error('Not implemented'); }
  async getState(runId) { throw new Error('Not implemented'); }
}
```
