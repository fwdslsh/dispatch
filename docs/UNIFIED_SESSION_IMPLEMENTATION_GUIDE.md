...
export class RunSessionManager {
	...
	async createRunSession({ kind, meta }) {
       ...
       VALUES(?, NULL, ?, 'starting', ?, ?, ?)`,
		...
		await this.db.run(`UPDATE sessions SET status='running', updated_at=? WHERE run_id=?`, [
			Date.now(),
			runId
		]);
...
export class RunSessionManager {
	...
	async nextSeqFor(runId) {
		const r = await this.db.get(
			`SELECT COALESCE(MAX(seq),0) as m FROM session_events WHERE run_id=?`,
			[runId]
		);
		return (r?.m ?? 0) + 1;
	}
	...
	async closeRunSession(runId) {
		...
		if (live) {
			...
			await this.db.run(`UPDATE sessions SET status='stopped', updated_at=? WHERE run_id=?`, [
				Date.now(),
				runId
			]);
		}
