	...
	$effect(() => {
		try {
			if (typeof localStorage !== 'undefined')
				localStorage.setItem(STORAGE.showSidebar, String(showSidebar));
		} catch {}
	});
</script>
			...
			<div class="claude-section">
				...
				{:else}
					<div class="empty-claude">
						...
						<p>
							Choose a project and session from the left to open an interactive Claude interface
						</p>
					</div>
				{/if}
			</div>
...
<style>
	...
	.error-icon {
		...
		font-family:
			'Apple Color Emoji', 'Segoe UI Emoji', 'Noto Color Emoji', 'Android Emoji', 'EmojiOne Color',
	...
	.mobile-sidebar {
		...
		background: linear-gradient(
			180deg,
			var(--bg),
			color-mix(in oklab, var(--bg) 95%, var(--surface) 5%)
		);
		z-index: 999;
