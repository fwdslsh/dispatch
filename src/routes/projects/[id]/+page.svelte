<script>
    ...
    function submitAuthToken() {
        ...
        vm.socket.emit("submit-auth-token", {
    ...
    async function handleAttachSession(sessionId) {
        ...
        if (success) {
            ...
            const sessionInfo = [...vm.sessions].find(s => s.id === sessionId || s.sessionId === sessionId);
...
<style>
    ...
    .sessions-panel h3, .sessions-panel h4 {
        margin: 0;
