...
export async function GET({ url, locals }) {
  ...
  return new Response(JSON.stringify({ sessions: all }), { headers: { 'content-type': 'application/json' }});
}
...
export async function POST({ request, locals }) {
  ...
  if (type === 'pty') {
    ...
    locals.sessions.bind(id, d); await locals.workspaces.rememberSession(workspacePath, d);
