<script>
	...
	let customSystemPrompt = $state(settings.customSystemPrompt ?? settingsService.get('claude.customSystemPrompt', ''));
	let appendSystemPrompt = $state(settings.appendSystemPrompt ?? settingsService.get('claude.appendSystemPrompt', ''));
	let maxTurns = $state(settings.maxTurns ?? settingsService.get('claude.maxTurns', null));
	let maxThinkingTokens = $state(settings.maxThinkingTokens ?? settingsService.get('claude.maxThinkingTokens', null));
	let fallbackModel = $state(settings.fallbackModel ?? settingsService.get('claude.fallbackModel', ''));
	...
	let continueConversation = $state(
		(settings.continue ?? settings.continueConversation) ?? settingsService.get('claude.continueConversation', false)
	);
	...
	let executableArgs = $state(settings.executableArgs ?? settingsService.get('claude.executableArgs', ''));
	let allowedTools = $state(settings.allowedTools ?? settingsService.get('claude.allowedTools', ''));
	let disallowedTools = $state(settings.disallowedTools ?? settingsService.get('claude.disallowedTools', ''));
	let additionalDirectories = $state(settings.additionalDirectories ?? settingsService.get('claude.additionalDirectories', ''));
	let strictMcpConfig = $state(settings.strictMcpConfig ?? settingsService.get('claude.strictMcpConfig', false));
			...
			<div class="setting-group">
				<label for="claude-model" class="setting-label">
					...
					{#if mode === 'session' && !model && settingsService.get('claude.model')}
						<span class="default-hint" title="Using global default: {settingsService.get('claude.model')}">🔧</span>
					{/if}
				</label>
