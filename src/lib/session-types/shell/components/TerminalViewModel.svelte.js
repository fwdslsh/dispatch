...
import { TerminalSocketService } from '$lib/session-types/shell/utils/terminal-socket.js';
import { TerminalSessionService } from '$lib/session-types/shell/utils/terminal-session.js';
import { TerminalConfigurationService } from '$lib/session-types/shell/utils/terminal-configuration.js';
import { createCleanupManager } from '$lib/shared/utils/cleanup-manager.js';
...
export class TerminalViewModel {
	...
	async initializeTerminal(terminal, options = {}) {
		try {
			...
			if (this.configService && this.configService.isTerminalInitialized()) {
				console.debug(
					'TerminalViewModel: Terminal already initialized, setting up input handling for this instance'
				);
			...
			if (this.queuedOutput && this.queuedOutput.length > 0 && !historyLoaded) {
				console.debug(
					'TerminalViewModel: Processing queued output:',
					this.queuedOutput.length,
					'items'
				);
				...
				console.debug(
					'TerminalViewModel: Skipping queued output to avoid duplication with loaded history'
				);
				this.queuedOutput = [];
			}
	...
	handleSocketOutput(output) {
		...
		if (data.includes('\n') || data.includes('\r')) {
			...
			let cleanOutput = this.currentOutputBuffer.replace(/\r\n/g, '\n').replace(/\r/g, '\n').trim();
