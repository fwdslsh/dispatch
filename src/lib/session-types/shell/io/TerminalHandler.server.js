...
export class TerminalHandler extends BaseHandler {
	...
	setupEventHandlers(socket) {
		socket.on(
			'terminals:input',
			this.authHandler.withAuth(this.handleInput.bind(this, socket), socket)
		);
		socket.on(
			'terminals:resize',
			this.authHandler.withAuth(this.handleResize.bind(this, socket), socket)
		);
		socket.on(
			'terminals:status',
			this.authHandler.withAuth(this.handleStatus.bind(this, socket), socket)
		);
	}
	...
	handleResize(socket, dims) {
		try {
			...
			console.log(
				`[TERMINAL] Resizing session ${sessionId} to ${cols}x${rows} from socket ${socket.id}`
			);
