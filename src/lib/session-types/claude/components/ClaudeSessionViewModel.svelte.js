...
export class ClaudeSessionViewModel {
  ...
  async sendMessage(messageText) {
    ...
    try {
      ...
      this.#socket.emit('claude:send-message', {
  ...
  clearChat() {
    ...
    this.#socket.emit('claude:clear-chat', {
      sessionId: this.sessionId
    }, (response) => {
      if (response.success) {
  ...
  endSession() {
    ...
    this.#socket.emit('claude:end-session', {
  ...
  #setupSocketHandlers() {
    ...
    this.#socket.on('claude:session-created', (data) => {
      ...
      this.#onSessionCreated?.({ sessionId: data.sessionId, type: 'claude', projectId: data.projectId });
    });
  ...
  #createClaudeSession() {
    ...
    this.#socket.emit('claude:init-session', {
