    ...
    try {
      ...
      if (fs.existsSync(tunnelUrlPath)) {
        ...
        if (callback) {
          callback({ 
            success: false, 
            error: 'No public tunnel active. Start the server with tunnel enabled to get a public URL.' 
          });
        }
      }
    } catch (error) {
...
export function registerUtilityHandlers(socket, handlers, requireAuth) {
  ...
  const protectedEvents = ['get-public-url', 'getPublicUrl', 'get-server-status', 'getServerStatus'];
...
export function createAndRegisterUtilityHandlers(io, socket, requireAuth) {
  ...
  console.log(`[UTILITY] Registered ${Object.keys(handlers).length} utility handlers for socket ${socket.id}`);
