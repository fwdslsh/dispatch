...
export function createAuthContext() {
  ...
  const isAuthenticating = $derived(() => auth.state === AUTH_STATES.AUTHENTICATING || auth.state === AUTH_STATES.CHECKING);
  ...
  const isClaudeAuthenticating = $derived(() => 
    claudeAuth.state === AUTH_STATES.AUTHENTICATING || 
    claudeAuth.state === AUTH_STATES.CHECKING
  );
...
export function getAuthContext() {
  ...
  if (!context) {
    throw new Error('Auth context not found. Make sure createAuthContext() is called in a parent component.');
  }
  return context;
}
