...
export class TerminalHistoryService {
  ...
  trimIfNeeded() {
    ...
    if (this.history.length > this.config.MAX_HISTORY_ENTRIES) {
      ...
      console.debug(`[TerminalHistory] Trimmed ${entriesToRemove} entries, now have ${this.history.length}`);
    }
    ...
    if (totalSize > this.config.MAX_BUFFER_LENGTH) {
      ...
      console.debug(`[TerminalHistory] Trimmed by size, now have ${this.history.length} entries, ${totalSize} chars`);
    }
  }
  ...
  save() {
    try {
      ...
      if (success) {
        ...
        console.debug(`[TerminalHistory] Saved ${this.history.length} entries for session ${this.sessionId}`);
      }
      return success;
    } catch (error) {
  ...
  load() {
    try {
      ...
      if (data && typeof data === 'object') {
        ...
        console.debug(`[TerminalHistory] Loaded ${this.history.length} entries for session ${this.sessionId}`);
        return this.currentBuffer;
      }
    } catch (error) {
  ...
  import(data) {
    try {
      ...
      console.debug(`[TerminalHistory] Imported ${this.history.length} entries for session ${this.sessionId}`);
      return true;
    } catch (error) {
