...
export class TerminalSessionService {
  ...
  async createSessionInProject(projectId, options = {}) {
    try {
      ...
      if (response && response.sessionId) {
        ...
        console.debug('TerminalSessionService: Session created in project:', this.sessionId, 'project:', projectId);
        return true;
      } else {
        this.status = 'disconnected';
        return false;
  ...
  initializeHistoryService() {
    if (this.sessionId && !this.historyService) {
      ...
      console.debug('TerminalSessionService: History service initialized for session:', this.sessionId);
    }
  }
