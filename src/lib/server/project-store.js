...
/**
 * Migration helper: Convert existing sessions to projects
 * @param {Array} existingSessions - Array of session objects from session-store
 * @returns {number} Number of projects created
 */
export function migrateSessionsToProjects(existingSessions) {
  ...
  for (const session of existingSessions) {
    ...
    if (data.projects.some(p => p.id === session.id)) {
      continue;
    }

    // Create project from session
    const project = {
      id: session.id, // Keep same ID for compatibility
      name: session.name || `Project ${session.id.slice(0, 8)}`,
      description: 'Migrated from session',
      createdAt: new Date().toISOString(),
      sessions: [{
        id: randomUUID(), // New session ID for the PTY within this project
        name: 'Main Session',
        type: 'pty',
        status: 'stopped', // Since we're migrating, assume sessions are stopped
        createdAt: new Date().toISOString()
      }]
    };
