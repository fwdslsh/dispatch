...
class ClaudeAuthManager {
  constructor() {
    /** @type {Map<string, { p: any, buffer: string, startedAt: number, awaitingCode: boolean }>} */
  ...
  start(socket) {
    try {
        ...
        try {
          ...
          if (state.codeSubmitted) {
            ...
            if (
              ...
              state.finished = true;
              try { socket.emit(SOCKET_EVENTS.CLAUDE_AUTH_COMPLETE, { success: true }); } catch {}
              logger.info('CLAUDE', 'Auth flow reported success; terminating PTY');
              try { state.p.kill(); } catch {}
              return;
            }
            if (lower.includes('invalid') || lower.includes('expired') || lower.includes('error')) {
              state.finished = true;
              try { socket.emit(SOCKET_EVENTS.CLAUDE_AUTH_ERROR, { success: false, error: 'Authorization code rejected' }); } catch {}
          ...
          if (url) {
            ...
            if (!state.urlEmitted) {
              ...
              try { socket.emit(SOCKET_EVENTS.CLAUDE_AUTH_URL, payload); } catch {}
      ...
      try { socket.emit(SOCKET_EVENTS.CLAUDE_AUTH_ERROR, { success: false, error: String(error?.message || error) }); } catch {}
      return false;
    }
  }
  ...
  submitCode(socket, code) {
    ...
    if (!state || !state.p) {
      try { socket.emit(SOCKET_EVENTS.CLAUDE_AUTH_ERROR, { success: false, error: 'No auth session active' }); } catch {}
      return false;
    }
    try {
      ...
      setTimeout(() => {
        try { if (!state.finished) state.p.write('\r'); } catch {}
      }, 250);
