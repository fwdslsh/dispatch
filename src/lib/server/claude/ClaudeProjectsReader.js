...
export class ClaudeProjectsReader {
	constructor(projectsDir = null) {
		...
		this.projectsDir =
			projectsDir || path.join(process.cwd(), '.dispatch-home', '.claude', 'projects');
	}
	...
	async getSessionMetadata(projectDir, sessionId) {
		...
		return {
			...
			firstMessage: firstUserMessage?.message?.content
				? this.truncateText(firstUserMessage.message.content, 100)
				: null,
		...
		const projects = await Promise.all(
			projectDirs.map(async (projectDir) => {
				...
				return {
					...
					lastActivity:
						sessions.length > 0
							? Math.max(...sessions.map((s) => new Date(s.endTime || s.startTime || 0).getTime()))
							: null
				};
			})
		);
	...
	async searchSessions(query, options = {}) {
		const { maxResults = 50, includeContent = false, projectFilter = null } = options;
	...
	async getStatistics() {
		...
		return {
			...
			sessionsToday: allSessions.filter((s) => new Date(s.endTime || s.startTime) > oneDayAgo)
				.length,
			sessionsThisWeek: allSessions.filter((s) => new Date(s.endTime || s.startTime) > oneWeekAgo)
				.length,
			averageSessionLength: this.calculateAverageSessionLength(allSessions),
			mostActiveProject:
				projects.reduce((max, p) => (p.sessionCount > (max?.sessionCount || 0) ? p : max), null)
					?.workspacePath || null
		};
	}
	...
	calculateAverageSessionLength(sessions) {
		const durationsWithValues = sessions.map((s) => s.duration).filter((d) => d !== null && d > 0);
