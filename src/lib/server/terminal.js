...
export class TerminalManager {
	...
	async createSessionInProject(projectId, opts = {}) {
		...
		console.log(
			`Created session ${sessionId} (${sessionName}) in project ${projectId} at ${sessionWorkingDir}`
		);
	...
	appendToBuffer(sessionId, data) {
		...
		if (buffer) {
			...
			if (totalSize > this.maxBufferSize) {
				...
				while (
					buffer.length > 0 &&
					buffer.reduce((sum, chunk) => sum + chunk.length, 0) >
						this.maxBufferSize * TERMINAL_CONFIG.BUFFER_TRIM_RATIO
				) {
					buffer.shift();
				}
			}
	...
	renameSession(sessionId, newName) {
		...
		const existingNames = storageManager
			.getAllSessionNames()
			.filter((name) => name !== metadata.name);
