...
export function createModularSocketHandler(io) {
	return (socket) => {
		...
		const sessionHandlers = createAndRegisterSessionHandlers(
			io,
			socket,
			requireAuth,
			socketSessions
		...
		const totalHandlers =
			Object.keys(projectHandlers).length +
			Object.keys(sessionHandlers).length +
			Object.keys(utilityHandlers).length +
			2; // +2 for auth and disconnect

		console.log(
			`[MODULAR] Socket ${socket.id} initialized with ${totalHandlers} handlers across 4 modules`
		);
...
export function broadcastToAuthenticated(io, event, data) {
	...
	console.log(
		`[MODULAR] Broadcasted ${event} to ${authenticatedSockets.size} authenticated sockets`
	);
}
