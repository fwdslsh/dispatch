...
export class WorkspaceManager {
  ...
  async init() {
    ...
    try { this.index = JSON.parse(await fs.readFile(this.indexFile, 'utf8')); }
  ...
  async rememberSession(dir, sessionDescriptor) {
    ...
    if (i >= 0) ws.sessions[i] = sessionDescriptor; else ws.sessions.push(sessionDescriptor);
    ws.lastActive = Date.now();
    await this.#persist();
  }
  async getIndex() { return this.index; }
  async #persist() { await fs.writeFile(this.indexFile, JSON.stringify(this.index, null, 2)); }
}
