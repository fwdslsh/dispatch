...
class ClaudeAuthContext {
	 ...
	 */
	async getAuthStatus(maxAge = 60000) {
		// 1 minute default
		const now = new Date();
		const shouldRefresh = !this.lastChecked || now - this.lastChecked > maxAge;
...
export function getClaudeAuthContext() {
	...
	if (!context) {
		throw new Error(
			'Claude auth context not found. Make sure to call createClaudeAuthContext() in a parent component.'
		);
	}
