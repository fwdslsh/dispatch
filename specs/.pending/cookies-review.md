...
io.use((socket, next) => {
	...
	const checkInterval = setInterval(async () => {
		if (socket.data.authMethod === 'session_cookie') {
			const valid = await services.sessionManager.validateSession(socket.data.session.id);
...
<script>
	// Clean login form, no localStorage detection // No "migrate from old auth" messaging // Just
	standard login flow
</script>;
```
