...
describe('ClaudeCommandCache', () => {
	it('returns cached commands while entry is fresh', async () => {
		...
		const first = await cache.getOrFetch({ cwd: '/work', pathToClaudeCodeExecutable: '/bin/claude' }, fetcher);
		expect(first).toEqual({ commands: ['run'], fromCache: false });
		expect(fetcher).toHaveBeenCalledTimes(1);

		const second = await cache.getOrFetch({ cwd: '/work', pathToClaudeCodeExecutable: '/bin/claude' }, fetcher);
	...
	it('refreshes commands after TTL expires', async () => {
		...
		const result = await cache.getOrFetch({ cwd: '/work', pathToClaudeCodeExecutable: '/bin/claude' }, fetcher);
