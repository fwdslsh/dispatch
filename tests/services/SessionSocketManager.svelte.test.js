...
const { default: sessionSocketManager } = await import('../../src/lib/components/SessionSocketManager.js');
...
describe('SessionSocketManager', () => {
	...
	describe('Reconnection Logic', () => {
		...
		it('should handle reconnection attempts with proper events', () => {
			...
			const reconnectAttemptHandler = socket.on.mock.calls.find(call => call[0] === 'reconnect_attempt')?.[1];
	...
	describe('Session Focus and Catch-up', () => {
		...
		it('should check for pending messages', async () => {
			...
			expect(socket.emit).toHaveBeenCalledWith('session.status', {
				key: 'testkey12345',
				sessionId
			}, expect.any(Function));
		});
