...
test.describe('Comprehensive Session Testing', () => {
	test('Projects display on initial load and both session types work', async ({ page }) => {
		...
		const createSessionBtn = page
			.locator('button:has-text("Create Session"), button:has-text("New Session")')
			.first();
		if (await createSessionBtn.isVisible()) {
			...
			console.log(
				'Create session button not found - checking for alternative ways to create sessions'
			);
	...
	test('Claude Code session creation and functionality', async ({ page }) => {
		...
		const modeSelectors = await page
			.locator('select, input[name*="mode"], button[data-mode="claude"]')
			.count();
		if (modeSelectors > 0) {
			...
			const claudeOption = page
				.locator('option[value="claude"], button[data-mode="claude"]')
				.first();
