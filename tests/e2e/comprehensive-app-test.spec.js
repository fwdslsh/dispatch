...
test.describe('Comprehensive Application Tests', () => {
    ...
    test('should show Claude authentication interface when Claude Agent is selected', async ({ page }) => {
        ...
        const claudeButton = page.locator('button:has-text("Claude"), button:has-text("Claude AI"), .session-btn.claude');
        if (await claudeButton.count() > 0) {
            ...
            const authButton = page.locator('button:has-text("Start Authentication"), button:has-text("🚀"), button:has-text("setup-token")');
    ...
    test('should handle session creation and management', async ({ page }) => {
        ...
        const newSessionButton = page.locator('button:has-text("New Session"), button:has-text("Shell"), .session-btn.shell');
...
test.describe('Claude Authentication Specific Tests', () => {
    ...
    test('should provide claude setup-token integration', async ({ page }) => {
        ...
        const claudeOptions = page.locator('select option[value="claude"], button:has-text("Claude"), .claude');
        ...
        if (await claudeOptions.count() > 0) {
            ...
            if (await sessionTypeSelector.count() > 0) {
                ...
                const authElements = page.locator('button:has-text("setup-token"), button:has-text("Start Authentication"), .claude-auth-status');
