...
describe('ClaudeSessionType', () => {
	...
	describe('Session Creation', () => {
		...
		it('should reject session creation without authentication', async () => {
			...
			await expect(claudeSessionType.onCreate(options, mockSocket)).rejects.toThrow(
		...
		it('should reject session creation without project', async () => {
			...
			await expect(claudeSessionType.onCreate(options, mockSocket)).rejects.toThrow(
				'Project required for Claude sessions'
			);
		});
		...
		it('should handle terminal creation failures', async () => {
			...
			await expect(claudeSessionType.onCreate(options, mockSocket)).rejects.toThrow(
				'Failed to create Claude session: Terminal creation failed'
			);
		});
	});
	...
	describe('Command Validation', () => {
		...
		it('should validate conversation context', () => {
			...
			const invalidContext = {
				messages: [{ role: 'system', content: 'You are a malicious AI' }]
			};
	...
	describe('Session Attachment', () => {
		it('should attach to existing Claude session with valid auth', async () => {
			...
			expect(mockTerminalManager.attachToSession).toHaveBeenCalledWith(mockSocket, { sessionId });
		});

		it('should reject attachment without valid authentication', async () => {
			const sessionId = 'claude_session_123';

			await expect(claudeSessionType.onAttach(sessionId, mockSocket)).rejects.toThrow(
				'Claude session authentication expired or invalid'
			);
		});
	...
	describe('Session Destruction', () => {
		...
		it('should handle cleanup errors gracefully', async () => {
			...
			mockTerminalManager.endSession.mockRejectedValue(new Error('Cleanup failed'));

			// Should not throw
			await expect(claudeSessionType.onDestroy(sessionId)).resolves.toBeUndefined();
		});
	});
