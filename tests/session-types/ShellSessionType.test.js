...
} from '../../src/lib/shared/utils/session-utils.js';
...
describe('ShellSessionType', () => {
	...
	describe('Session Creation', () => {
		...
		it('should handle terminal creation failures', async () => {
			...
			mockTerminalManager.createSession.mockRejectedValue(new Error('Failed to spawn terminal'));

			await expect(shellSessionType.onCreate(sessionOptions, mockSocket)).rejects.toThrow(
				'Failed to create shell session: Failed to spawn terminal'
			);
		});
	...
	describe('Session Attachment', () => {
		it('should attach to existing shell session', async () => {
			...
			expect(mockTerminalManager.attachToSession).toHaveBeenCalledWith(sessionId, mockSocket);
		...
		it('should handle attachment errors', async () => {
			...
			mockTerminalManager.attachToSession.mockRejectedValue(new Error('Session not found'));

			await expect(shellSessionType.onAttach(sessionId, mockSocket)).rejects.toThrow(
				'Session not found'
			);
		});
	});
	...
	describe('Session Destruction', () => {
		...
		it('should handle cleanup errors gracefully', async () => {
			...
			mockTerminalManager.endSession.mockRejectedValue(new Error('Process already terminated'));
	...
	describe('Terminal Operations', () => {
		...
		it('should validate resize dimensions', () => {
			...
			expect(() => shellSessionType.handleResize(sessionId, invalidDimensions)).toThrow(
