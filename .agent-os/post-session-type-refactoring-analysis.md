...
<!-- Others mix patterns: -->
const {(session = {})} = $props(); let {((socket = null), (readonly = false))} = $props();
```
...
<!-- After: -->
let {(onSessionAction, onError, onViewChanged)} = $props(); // Use callback props directly
```
...
src/lib/components/session-types/ ├── SessionHeader.svelte # Standardized header ├── SessionControls.svelte
# Common control patterns ├── SessionStatusBar.svelte # Status display └── SessionMetadata.svelte # Metadata
display patterns
```
