...
<style>
  ...
  .toggle { padding: .45rem .65rem; font-size: .95rem; }
  .list {
  ...
  .item {
    ...
    padding: .6rem .7rem; border: 0; border-bottom: 1px solid #f1f1f1; background: transparent;
  }
  .item:hover { background: #f6f8fa; }
  .desc { font-size: .85rem; color: #555; }
  .empty { padding: .6rem .7rem; font-style: italic; }
</style>
```
...
<style>
  ...
  .bar { display: flex; align-items: center; gap: .5rem; padding: .5rem; border-top: 1px solid #ddd; background: #fff; }
  @media (min-width: 720px) {
    .chat { border-left: 1px solid #eee; border-right: 1px solid #eee; }
  }
</style>
```
