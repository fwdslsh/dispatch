...
test.describe('Working Directory Functionality', () => {
  ...
  test('should show directory picker for session creation', async ({ page }) => {
    ...
    if (await createButton.isVisible()) {
      ...
      if (hasWorkingDirSelect) {
        ...
        const hasRootOption = optionTexts.some(text => 
          text?.includes('/') || text?.includes('root') || text?.includes('workspace')
        );
    ...
    if (await createButton.isVisible()) {
      ...
      if (await submitButton.isVisible()) {
        ...
        if (await terminal.isVisible()) {
          ...
          const hasProjectPath = terminalContent?.includes('projects') || 
  ...
  test('should show directory listing for project', async ({ page }) => {
    ...
    const directoryList = page.locator('.directory-list, .file-browser, [data-testid="directory-list"]');
    const directoryItems = page.locator('.directory-item, .file-item, [data-testid="directory-item"]');
  ...
  test('should handle nested directory navigation', async ({ page }) => {
    ...
    if (await createButton.isVisible()) {
      ...
      if (await workingDirSelect.isVisible()) {
        ...
        const nestedOptions = optionTexts.filter(text => 
    ...
    if (await createButton.isVisible()) {
      ...
      if (await submitButton.isVisible()) {
        ...
        if (await sessionItems.count() > 0) {
          ...
          const attachButton = lastSession.locator('button:has-text("Attach"), button:has-text("Open")');
  ...
  test('should show working directory in session list', async ({ page }) => {
    ...
    if (sessionCount > 0) {
      ...
      if (await sessionInfo.isVisible()) {
        ...
        const hasDirectoryInfo = infoText?.includes('/') || 
                                 infoText?.includes('workspace') ||
                                 infoText?.includes('dir');
  ...
  test('should handle directory listing API', async ({ page }) => {
    ...
    const directoryButton = page.locator('button:has-text("Browse"), button:has-text("Files"), [data-testid="browse-files"]');
