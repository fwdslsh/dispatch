...
test.describe('Terminal Sessions', () => {
  ...
  test('should display project sessions page', async ({ page }) => {
    ...
    const createSessionButton = page.locator('button:has-text("Create"), button:has-text("New Session"), [data-testid="create-session"]');
  ...
  test('should create a shell session', async ({ page }) => {
    ...
    // Fill session form
    const nameInput = page.locator('input[placeholder*="name" i], [data-testid="session-name"]').first();
    if (await nameInput.isVisible()) {
  ...
  test('should display active sessions list', async ({ page }) => {
    ...
    if (hasSessions) {
      ...
      if (sessionCount > 0) {
        ...
        const attachButton = firstSession.locator('button:has-text("Attach"), button:has-text("Open")');
        const deleteButton = firstSession.locator('button:has-text("Delete"), button:has-text("End")');
  ...
  test('should handle session termination', async ({ page }) => {
    ...
    if (sessionCount > 0) {
      const firstSession = sessionItems.first();
      const deleteButton = firstSession.locator('button:has-text("Delete"), button:has-text("End"), .delete-btn');
