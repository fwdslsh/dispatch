...
test.describe('Mobile UX Features', () => {
			...
			if (await createButton.isVisible()) {
				...
				if (await nameInput.isVisible()) {
					...
					if (await submitButton.isVisible()) {
						...
						const keyboardToolbar = page.locator(
							'.keyboard-toolbar, [data-testid="keyboard-toolbar"]'
						);
	...
	test('should handle mobile command palette', async ({ page }) => {
		...
		const commandPaletteButton = page.locator(
			'button:has-text("Commands"), [data-testid="command-palette"], .command-palette-btn'
		);
