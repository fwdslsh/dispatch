...
test.describe('Visual Consistency Review', () => {
  ...
  test('Projects page - Create session modal', async ({ page }) => {
    ...
    const createButtons = page.locator('button:has-text("Claude"), button:has-text("Terminal"), button:has-text("New"), button[class*="add"]');
    ...
    for (let i = 0; i < await createButtons.count(); i++) {
      ...
      if (await button.isVisible()) {
        ...
        if (await modal.isVisible()) {
          ...
          // Close modal
          const closeModal = page.locator('button[aria-label*="close" i], button:has-text("Cancel")');
