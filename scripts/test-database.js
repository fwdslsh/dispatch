...
async function testDatabase() {
	...
	try {
		...
		// Test event logging
		await testDb.addSessionEvent('test-session-1', 'socket-123', 'test.event', 'in', {
			message: 'Hello'
		});
		...
		if (events.length === 1 && events[0].event_type === 'test.event') {
			...
			throw new Error(
				`Event retrieval failed - expected 1 event with type 'test.event', got ${events.length} events`
			);
		}
		...
		await testDb.addWorkspaceSession(
			'session-123',
			'/test/workspace',
			'claude',
			'claude-456',
